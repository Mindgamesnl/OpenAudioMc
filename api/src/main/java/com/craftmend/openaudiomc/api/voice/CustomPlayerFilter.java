 ...
 * against each other.
 *<br />
 * The easiest example is a game plugin, which will not let players hear each other if they are on different teams, or moderation
 * where you would prevent two players from hearing each other if one of them has been muted/punished.
 *<br />
 ...
 * <br />
 * Please note that a filter is only called in one direction, meaning that if its called for playerA -> playerB, it will not be called for playerB -> playerA.
 * This is done as an optimization to prevent poor scaling with a large amount of players.
 * <br />
 * Because of that, filters should only be used to filter players that should be considered for voicechat.
 ...
 * (once for each player) and allow you to cancel the event if you don't want the players to be able to hear each other.
 *
 * @since 6.10.0
