...
public class RegionSelectionGui extends Menu {
    ...
    public RegionSelectionGui(Player opener) {
        ...
        if (applicableRegions.isEmpty()) {
            opener.sendMessage(OpenAudioMc.getInstance().getCommandModule().getCommandPrefix() + ChatColor.RED + "There are no OpenAudioMc regions registered on your current location, so there's nothing to display in the GUI.");
            return;
        }
