...
public class DefaultNetworkingService extends NetworkingService {
    ...
    private void init() {
        ...
        // set tick tack
        OpenAudioMc.resolveDependency(TaskService.class).scheduleSyncRepeatingTask(() -> clientMap.values().forEach(clientConnection -> clientConnection.getSession().tick()), 20, 20);
        try {
    ...
    public void connectIfDown() {
        try {
            if (!connectLock.tryLock(30, TimeUnit.SECONDS)) return;
