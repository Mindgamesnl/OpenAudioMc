...
public class SpigotCommandService extends Service {
    ...
    public void onEnable() {
        ...
        commandService.registerSubCommands(CommandContext.OPENAUDIOMC, new RegionsSubCommand(openAudioMcSpigot), new SpeakersSubCommand(openAudioMcSpigot), new ShowSubCommand(openAudioMcSpigot), new AliasSubCommand(), new VoiceSubCommand(), new PlaylistSubCommand());
        ...
        if (OpenAudioMc.getService(ServerService.class).getVersion().getRevision() > ServerVersion.LEGACY.getRevision()) {
            openAudioMcSpigot.getServer().getPluginManager().registerEvents(new CommandTranslationMiddleware(), openAudioMcSpigot);
        }
    }
