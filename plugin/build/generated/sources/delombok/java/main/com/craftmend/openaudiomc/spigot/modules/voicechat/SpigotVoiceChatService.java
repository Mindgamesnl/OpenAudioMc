...
public class SpigotVoiceChatService extends Service {
    ...
    public void onEnable() {
        ...
        if (StorageKey.SETTINGS_VOICECHAT_VICINITY_REMINDER_ENABLED.getBoolean()) {
            taskService.scheduleAsyncRepeatingTask(new PlayerVicinityMessageTask(OpenAudioMc.getService(SpigotPlayerService.class)), 40, 40);
        }
        ...
        eventApi.registerHandler(VoicechatPeerTickEvent.class, handler -> {
            ...
            for (ClientConnection client : networkingService.getClients()) {
                ...
                // remove mods
                manager.getCurrentProximityAdditions().removeIf(uuid -> clientFromId(uuid).isModerating());
                manager.getCurrentProximityDrops().removeIf(uuid -> clientFromId(uuid).isModerating());
                ...
                if (!manager.getCurrentProximityAdditions().isEmpty()) {
                    ...
                    if (manager.getCurrentProximityAdditions().size() == 1) {
                        ...
                        if (other != null) {
                            sendMessage(client.getUser(), Platform.translateColors(StorageKey.MESSAGE_VC_USER_ADDED.getString().replace("%name", other.getOwner().getName())));
                        }
                    } else {
                        // do multiple
                        MultiNameReference mnr = new MultiNameReference(manager.getCurrentProximityAdditions());
                        sendMessage(client.getUser(), Platform.translateColors(StorageKey.MESSAGE_VC_USERS_ADDED.getString().replace("%count", mnr.getOtherCount() + "").replace("%name", mnr.getFirstName())));
                    }
                ...
                if (!manager.getCurrentProximityDrops().isEmpty()) {
                    ...
                    if (manager.getCurrentProximityDrops().size() == 1) {
                        ...
                        if (other != null) {
                            sendMessage(client.getUser(), Platform.translateColors(StorageKey.MESSAGE_VC_USER_LEFT.getString().replace("%name", other.getOwner().getName())));
                        }
                    } else {
                        // do multiple
                        MultiNameReference mnr = new MultiNameReference(manager.getCurrentProximityDrops());
                        sendMessage(client.getUser(), Platform.translateColors(StorageKey.MESSAGE_VC_USERS_LEFT.getString().replace("%count", mnr.getOtherCount() + "").replace("%name", mnr.getFirstName())));
                    }
    ...
    private class MultiNameReference {
        ...
        public MultiNameReference(final String firstName, final int otherCount) {
    ...
    public PlayerPeerTicker getPeerTicker() {
        return this.peerTicker;
    }
}
