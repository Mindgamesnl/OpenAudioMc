...
public class VoiceSocket extends WebSocketListener {
    private static final OkHttpClient CLIENT = new OkHttpClient.Builder().connectTimeout(2, TimeUnit.SECONDS).build();
    ...
    public boolean start() {
        ...
        RestRequest<NoResponse> urlBuilder = new RestRequest<>(NoResponse.class, Endpoint.VOICE_BUS).setQuery("publicKey", authenticationService.getServerKeySet().getPublicKey().getValue()).setQuery("privateKey", authenticationService.getServerKeySet().getPrivateKey().getValue()).setQuery("password", this.password);
        if (StorageKey.SETTINGS_AUTO_RECONNECT.getBoolean()) {
        ...
        Request request = new Request.Builder().url(ebUri).build();
        this.isReady = false;
    ...
    public boolean isAnnouncedShutdown() {
