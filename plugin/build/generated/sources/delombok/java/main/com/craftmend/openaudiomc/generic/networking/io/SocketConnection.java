...
public class SocketConnection {
    ...
    private final SocketDriver[] drivers = new SocketDriver[] {new NotificationDriver(), systemDriver, new ClientDriver()};
    ...
    public void setupConnection() {
        ...
        if (StorageKey.SETTINGS_AUTO_RECONNECT.getBoolean()) {
            opts.query = String.format("type=server&private=%s&public=%s&reconnect=%s", keySet.getPrivateKey().getValue(), keySet.getPublicKey().getValue(), isReconnect ? "true" : "false");
        } else {
            opts.query = String.format("type=server&private=%s&public=%s", keySet.getPrivateKey().getValue(), keySet.getPublicKey().getValue());
        }
    ...
    public DefaultNetworkingService getParent() {
        return this.parent;
    }

    public RestRequest<RelayLoginResponse> getRelayLoginRequest() {
        return this.relayLoginRequest;
    }

    public RelayLoginResponse getPreviousLogin() {
