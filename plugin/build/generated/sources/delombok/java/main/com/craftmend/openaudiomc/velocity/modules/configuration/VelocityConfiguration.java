...
public class VelocityConfiguration implements Configuration {
    ...
    public String getString(StorageKey storageKey) {
        ...
        case CONFIG_FILE: 
            return ((mainConfig.getNode(path).isVirtual() ? "<unknown openaudiomc value " + storageKey.getPath() + ">" : mainConfig.getNode(path).getString()));
        default: 
    ...
    public void saveAll(boolean ignored) {
        try {
            ...
            YAMLConfigurationLoader.builder().setFile(config).build().save(mainConfig);
            YAMLConfigurationLoader.builder().setFile(data).build().save(dataConfig);
        } catch (IOException e) {
    ...
    private ConfigurationNode getFile(String filename) {
        ...
        YAMLConfigurationLoader yamlLoader = YAMLConfigurationLoader.builder().setFile(file).setFlowStyle(DumperOptions.FlowStyle.BLOCK).build();
        try {
    ...
    private void saveDefaultFile(String filename, boolean hard) {
        if (!OpenAudioMcVelocity.getInstance().getDataDir().exists()) OpenAudioMcVelocity.getInstance().getDataDir().mkdir();
        File file = new File(OpenAudioMcVelocity.getInstance().getDataDir(), filename);
        if (hard && file.exists()) {
            try {
