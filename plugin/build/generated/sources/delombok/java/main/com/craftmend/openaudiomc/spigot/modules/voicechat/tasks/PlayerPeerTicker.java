...
public class PlayerPeerTicker implements Runnable {
    ...
    public void run() {
        ...
        ClientConnection[] allClients = OpenAudioMc.getService(NetworkingService.class).getClients().stream().filter(c -> c.getRtcSessionManager().isReady()).toArray(ClientConnection[]::new);
        ...
        for (ClientConnection client : allClients) {
            ...
            if (!client.getRtcSessionManager().getBlockReasons().isEmpty()) {
                ...
                Stream<ClientConnection> pre =  // don't check players that are resetting
                ...
                // future checks
                // don't check combinations that failed
                Stream.of(allClients).filter(c -> !c.getSession().isResetVc()).filter(c -> c.getOwner().getUniqueId() != client.getOwner().getUniqueId()).filter(c -> client.isModerating() == c.isModerating() || client.isModerating() && !c.isModerating()).filter(c -> combinationChecker.getAndPutIfAbsent(client.getUser().getUniqueId(), c.getUser().getUniqueId(), false) == CombinationChecker.NOT_CHECKED).filter(c -> !client.getRtcSessionManager().getCurrentGlobalPeers().contains(c.getOwner().getUniqueId())) // exempt global peers
                ;
                // execute API filters
                applicableClients = filter.wrap(pre, player).collect(Collectors.toSet());
            }
            ...
            client.getRtcSessionManager().getCurrentProximityPeers().stream().filter(p -> p != client.getOwner().getUniqueId()).filter(uuid -> (client.getSession().isResetVc() || combinationChecker.stateIs(client.getOwner().getUniqueId(), uuid) != CombinationChecker.CHECKED_TRUE)).filter(uuid -> !client.getRtcSessionManager().getCurrentGlobalPeers().contains(uuid)).collect(Collectors.toSet())) {
                // unsubscribe these
    ...
    public void setFilter(final Filter<ClientConnection, Player> filter) {
        this.filter = filter;
    }
}
