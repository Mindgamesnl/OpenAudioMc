...
public class SpeakerHandler implements ITickableHandler {
    ...
    public void tick() {
        ...
        enteredSpeakers.forEach(entered -> {
            if (!isPlayingSpeaker(entered)) {
                ...
                packetQue.add(new PacketClientCreateSpeaker(new ClientSpeakerCreatePayload(cp)));
            }
        });
    ...
    public void forceDeleteSpeaker(String id, SpeakerType type, String source) {
        OpenAudioMc.getService(NetworkingService.class).send(spigotConnection.getClientConnection(), new PacketClientRemoveSpeaker(new ClientSpeakerDestroyPayload(new ClientSpeaker(Vector3.EMPTY, type, id, source, 0, 0, 0, false, false, false))));
    }
    ...
    private ClientSpeaker toClientSpeaker(ApplicableSpeaker speaker, int obstructions) {
        ...
        return new ClientSpeaker(speaker.getLocation(), speaker.getSpeakerType(), id, speaker.getSpeaker().getSource(), speaker.getSpeaker().getRadius(), speaker.getSpeaker().getMedia().getStartInstant(), obstructions, true, true, ExtraSpeakerOptions.OVERWRITE_REGIONS.isEnabledFor(speaker.getSpeaker()));
    }

    public SpeakerHandler(final Player player, final SpigotConnection spigotConnection) {
        this.player = player;
