...
public class SpigotAudioCommand implements CommandExecutor {
    private final CommandMiddleware[] commandMiddleware = new CommandMiddleware[] {new CatchLegalBindingMiddleware(), new CatchCrashMiddleware(), new CleanStateCheckMiddleware()};
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (commandSender instanceof Player) {
            ...
            if (args.length == 1) {
                OpenAudioMc.getService(NetworkingService.class).getClient(sua.getUniqueId()).getAuth().activateToken(sua, args[0]);
                return true;
            }
            Player sender = (Player) commandSender;
