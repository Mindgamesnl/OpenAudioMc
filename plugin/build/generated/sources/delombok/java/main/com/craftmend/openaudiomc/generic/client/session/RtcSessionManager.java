...
public class RtcSessionManager implements Serializable {
    ...
    public boolean requestLinkage(ClientConnection peer, boolean mutual, VoicePeerOptions options) {
        ...
        if (!peer.getRtcSessionManager().isReady()) return false;
        ...
        if (mutual && !peer.getRtcSessionManager().currentProximityPeers.contains(clientConnection.getOwner().getUniqueId())) {
            ClientPeerAddEvent event = (ClientPeerAddEvent) EventApi.getInstance().callEvent(new ClientPeerAddEvent(clientConnection, peer, options.clone()));
        ...
        if (currentProximityPeers.contains(peer.getOwner().getUniqueId())) return false;
        ClientPeerAddEvent event = (ClientPeerAddEvent) EventApi.getInstance().callEvent(new ClientPeerAddEvent(peer, clientConnection, options.clone()));
        if (!event.isCancelled()) {
    ...
    public void makePeersDrop() {
        for (ClientConnection peer : OpenAudioMc.getService(NetworkingService.class).getClients()) {
            if (peer.getOwner().getUniqueId() == clientConnection.getOwner().getUniqueId()) continue;
    ...
    public void forceUpdateLocation(Location location) {
        for (ClientConnection peer : OpenAudioMc.getService(NetworkingService.class).getClients()) {
            if (peer.getOwner().getUniqueId() == clientConnection.getOwner().getUniqueId()) continue;
            if (peer.getRtcSessionManager().currentProximityPeers.contains(clientConnection.getOwner().getUniqueId())) {
                peer.getRtcSessionManager().locationUpdateQueue.add(ClientRtcLocationUpdate.fromClientWithLocation(clientConnection, location, Vector3.from(peer)));
            }
        }
    }
    ...
    public boolean isVoicechatDeafened() {
        return this.isVoicechatDeafened;
    }

    public Set<UUID> getCurrentGlobalPeers() {
        return this.currentGlobalPeers;
    }

    public Set<UUID> getCurrentProximityPeers() {
        return this.currentProximityPeers;
    }
    ...
    public Set<UUID> getCurrentProximityAdditions() {
        return this.currentProximityAdditions;
    }

    public Set<UUID> getCurrentProximityDrops() {
        return this.currentProximityDrops;
    }
