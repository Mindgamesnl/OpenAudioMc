...
public class AuthenticationDriver {
    ...
    public Task<Boolean> activateToken(Authenticatable auth, String token) {
        ...
        OpenAudioMc.resolveDependency(TaskService.class).runAsync(() -> {
            ClientTokenRequestBody requestBody = new ClientTokenRequestBody("ACCOUNT", auth.getOwner().getName(), auth.getOwner().getUniqueId().toString(), auth.getAuth().getWebSessionKey(), service.getServerKeySet().getPublicKey().getValue(), (StorageKey.SETTINGS_TOKEN_AUTO_LOGIN.getBoolean() ? auth.getOwner().getIpAddress() : null), token);
    ...
    public Task<String> createPlayerSession(Authenticatable authenticatable) {
        ...
        OpenAudioMc.resolveDependency(TaskService.class).runAsync(() -> {
            ...
            // create request
            ClientTokenRequestBody requestBody = new ClientTokenRequestBody("ACCOUNT", authenticatable.getOwner().getName(), authenticatable.getOwner().getUniqueId().toString(), authenticatable.getAuth().getWebSessionKey(), service.getServerKeySet().getPublicKey().getValue(), (StorageKey.SETTINGS_TOKEN_AUTO_LOGIN.getBoolean() ? authenticatable.getOwner().getIpAddress() : null), null // unused here, this isn't reverse auth
            );
    ...
    public AuthenticationDriver(final AuthenticationService service) {
        this.service = service;
    }

    public ConcurrentHeatMap<UUID, String> getSessionCacheMap() {
