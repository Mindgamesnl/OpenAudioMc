...
public class ClientAuth implements Serializable, ClientBaseAuthentication {
    ...
    public void publishSessionUrl() {
        ...
        if (openAudioMc.getPlatform() == Platform.SPIGOT && OpenAudioMcSpigot.getInstance().getProxyModule().getMode() == OAClientMode.NODE) return;
        ...
        if (client.isConnected()) {
            client.getUser().sendMessage(translateColors(StorageKey.MESSAGE_CLIENT_ALREADY_CONNECTED.getString()));
            return;
        }
        ...
        sessionRequest.setWhenFinished(token -> {
            ...
            String msgText = translateColors(finalOurMessage.replace("{url}", url).replace("{token}", token));
    ...
    public ClientAuth(final ClientConnection client, final String webSessionKey, final String staticToken) {
        this.client = client;
        this.webSessionKey = webSessionKey;
        this.staticToken = staticToken;
    }

    public void setClient(final ClientConnection client) {
        this.client = client;
    }
}
