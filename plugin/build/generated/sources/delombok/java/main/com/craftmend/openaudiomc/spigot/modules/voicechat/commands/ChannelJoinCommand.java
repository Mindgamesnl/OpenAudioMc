...
public class ChannelJoinCommand extends SubCommand {
    public ChannelJoinCommand() {
        super("join");
        registerArguments(new Argument("<channel-name>", "Join a channel").addTabCompleteProvider(0, new VoiceChannelTabCompleteProvider()));
    ...
    public void onExecute(User sender, String[] args) {
        try {
            ...
            if (response != ChannelEnterResponse.OK) {
                throw new CommandError(response.getMessage().replace("{owner}", targetChannel.getCreator() != null ? targetChannel.getCreator().getName() : "unknown"));
            }
            targetChannel.addMember(sender);
            sender.sendMessage(Platform.translateColors(StorageKey.MESSAGE_VOICE_CHANNEL_JOINED.getString().replace("{channel}", channelName)));
