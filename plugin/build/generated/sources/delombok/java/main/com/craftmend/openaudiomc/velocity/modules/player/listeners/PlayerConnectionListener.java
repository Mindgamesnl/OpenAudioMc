...
public class PlayerConnectionListener {
    ...
    public void onSwitch(ServerPostConnectEvent event) {
        ...
        new VelocityProxyNode(currentServer.getServer()).sendPacket(new AnnouncePlatformPacket(OpenAudioMc.getService(AuthenticationService.class).getServerKeySet().getPublicKey().getValue(), Platform.VELOCITY));
        OpenAudioMc.getService(ProxyHostService.class).onServerSwitch(new VelocityUserAdapter(event.getPlayer()), from, new VelocityProxyNode(currentServer.getServer()));
    }
}
