...
public class ChannelInviteCommand extends SubCommand {
    ...
    public void onExecute(User sender, String[] args) {
        try {
            ...
            String invitationMessage = StorageKey.MESSAGE_VOICE_CHANNEL_INVITED.getString().replace("{channel}", channel.getName()).replace("{inviter}", sender.getName());
            ...
            targetClient.getUser().sendClickableCommandMessage(invitationMessage, "Click to join the channel", "channel invite use " + invitationId);
            sender.sendMessage(Platform.translateColors(StorageKey.MESSAGE_VOICE_CHANNEL_INVITATION_SENT.getString().replace("{player}", target.getName())));
            Bukkit.getScheduler().runTaskLater(OpenAudioMcSpigot.getInstance(), () -> {
                if (invitations.containsKey(invitationId)) {
                    invitations.remove(invitationId);
                    sender.sendMessage(Platform.translateColors(StorageKey.MESSAGE_VOICE_CHANNEL_INVITATION_EXPIRED.getString().replace("{player}", target.getName())));
                }
            }, 20 * 30);
