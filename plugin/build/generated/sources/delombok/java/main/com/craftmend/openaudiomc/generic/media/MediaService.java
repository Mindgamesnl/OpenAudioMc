...
public class MediaService extends Service {
    ...
    private final List<Runnable> resetTriggers = new ArrayList<>();
    @Inject
    ...
    public String process(String original) {
        ...
        for (String selector : urlMutations.keySet()) {
            if (original.startsWith(selector)) {
                for (UrlMutation urlMutation : urlMutations.get(selector)) {
                    if (!(urlMutation instanceof ForcedUrlMutation)) original = urlMutation.onRequest(original);
                }
            }
        }
        return original;
    ...
    public List<Runnable> getResetTriggers() {
