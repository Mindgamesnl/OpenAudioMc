...
public class PlaySubCommand extends SubCommand {
    public PlaySubCommand() {
        super("play", "p");
        registerArguments(new Argument("<selector> <source>", "Plays a sound for all the players in a selection", 0).addTabCompleteProvider(1, MediaTabcompleteProvider.getInstance()), new Argument("<selector> <source> <options>", "Plays a sound with configuration (like fade time, sync etc) for all players in a selection", 0).addTabCompleteProvider(1, MediaTabcompleteProvider.getInstance()));
    }
