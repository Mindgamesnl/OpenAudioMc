...
public class RedisService extends Service {
    ...
    public void onEnable() {
        ...
        if (Configuration.getString(StorageKey.REDIS_SENTINEL_MASTER_SET).isEmpty()) {
            uri = RedisURI.builder().withPassword(password).withHost(Configuration.getString(StorageKey.REDIS_HOST)).withPort(Configuration.getInt(StorageKey.REDIS_PORT)).withSsl(Configuration.getBoolean(StorageKey.REDIS_USE_SSL)).build();
        } else {
            final RedisURI.Builder builder = RedisURI.builder().withPassword(password).withSsl(Configuration.getBoolean(StorageKey.REDIS_USE_SSL)).withSentinelMasterId(Configuration.getString(StorageKey.REDIS_SENTINEL_MASTER_SET));
