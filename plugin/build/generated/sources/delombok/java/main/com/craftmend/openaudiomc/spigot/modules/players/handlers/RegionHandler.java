...
public class RegionHandler implements ITickableHandler {
    ...
    public void tick() {
        if (OpenAudioMcSpigot.getInstance().getRegionModule() != null) {
            ...
            if (StorageKey.SETTINGS_IGNORE_REGIONS_WHILE_IN_VEHICLE.getBoolean() && player.getVehicle() != null) {
                ...
                detectedRegions = OpenAudioMcSpigot.getInstance().getRegionModule().getRegionAdapter().getAudioRegions(player.getLocation());
            }
            ...
            enteredRegions.forEach(entered -> {
                if (!isPlayingRegion(entered)) {
                    ...
                    MediaUpdate mediaUpdate = new MediaUpdate(100, 100, entered.getFadeTime(), entered.getVolume(), true, entered.getMedia().getMediaId());
    ...
    public RegionHandler(final Player player, final SpigotConnection spigotConnection) {
        this.player = player;
