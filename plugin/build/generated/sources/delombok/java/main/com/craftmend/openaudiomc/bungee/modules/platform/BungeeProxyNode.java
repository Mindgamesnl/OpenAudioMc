    ...
    public void sendPacket(StandardPacket packet) {
        for (User onlineUser : getOnlineUsers()) {
            ...
            if (BungeeUtils.areEncodersReady((ProxiedPlayer) onlineUser.getOriginal())) {
                // send packet
                OpenAudioMcBungee.getInstance().getMessageHandler().sendPacket(new PacketPlayer((ProxiedPlayer) onlineUser.getOriginal()), packet);
                return;
            }
        }
    }

    public BungeeProxyNode(final ServerInfo serverInfo) {
